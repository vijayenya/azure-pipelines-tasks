...
export async function run(): Promise<void> {
    ...
    try {
        ...
        let projectFiles = CreateProjectFiles(nuGetConfigHelper.tempNugetConfigPath);

        try {
            for (const projectFile of projectFiles) {
                await dotNetRestoreAsync(dotnetPath, projectFile, packageName, packagesDirectory, packageVersion, configFile, noCache, verbosity);
            }
        } finally {
            credCleanup();
...
function getProjectFilePath(configPath: string): string {
    ...
    console.log("base path is: " + tempNuGetConfigBaseDir);
...
function dotNetRestoreAsync(dotnetPath: string, projectFile: string, packageName: string, packagesDirectory: string, version: string, configFile: string, noCache: boolean, verbosity: string): Q.Promise<number> {
